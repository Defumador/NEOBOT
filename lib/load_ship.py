...
def drag_items_to_cargo_hold():
    # click and drag first item stack in inventory to ship cargo hold,
    # function assumes
    ...
    if namefield_station_hangar is None:
        ...
        while cargo_hold is None:
            ...
            cargo_hold = pag.locateCenterOnScreen('./img/cargo_hold.bmp',
                                                  confidence = conf)
            time.sleep(1)
        # if found icons, click on first item in station hangar and drag
        # mouse to ship cargo hold
        if cargo_hold is not None:
            (namefield_station_hangarx,
             namefield_station_hangary) = namefield_station_hangar
...
def drag_items_to_special_hold():
    # click and drag first item stack in inventory to ship special hold,
    # function assumes
...
def load_ship_bulk():
    ...
    if items is None:
        ...
        time.sleep(
            2)  # after moving stack to cargo hold, wait and look for warnings
        ...
        if nospace == 0 and setquant == 0:
            ...
            return 2  # 2 indicating ship is loaded and hangar is completley
            # empty

        elif nospace == 1:  # if warning appears, look for additional cargo
            # hold
            ...
            if specialhold == 1:
                ...
                if specialholdwarning == 0 and setquant == 0 and nospace == 0:
                    docked.look_for_items()  # if no warnings, look for more
                    # items
                    if items == 0:
                        return 2
                    else:
                ...
                elif specialholdwarning == 0 and setquant == 1 and nospace \
                        == 0:
                    return 1

                else:  # if warning appears, try loading items individually
                    return 0  # 0 indicating ship cannot be fully loaded in
                    # bulk
            elif specialhold == 0:
                return 0
        elif setquant == 1:
...
def load_ship_individually():
    ...
    while items == 1:
        ...
        if nospace == 0 and setquant == 0:
            ...
            return 1  # 1 indicating ship is full but more items remain in
            # hangar

        # if warning appears but items are still present, look for
        # additional cargo hold
            ...
            if specialhold == 1:
                ...
                while specialholdwarning == 0 and setquant == 0 and nospace \
                        == 0:
                    drag_items_to_special_hold()
                    time.sleep(2)
                    specialholdwarning = docked.special_hold_warning()
                ...
                if items is None:
                    print(
                        'load_ship_individually -- done loading special hold')
                    return 2
                elif specialholdwarning == 1 or setquant == 1 or nospace == 1:
                    return 1  # 1 indicating ship is full but more items
                    # remain in hangar
            else:
                return 1  # 1 indicating ship is full but more items remain
                # in hangar
    if items is None:
...
def load_ship():
    ...
    if items == 1:
        ...
        if lsb == 2:
        ...
        elif lsb == 1:  # 1 indicating ship is full but more items remain in
            # hangar
            ...
            elif lsi == 1:  # 1 indicating ship is full but more items
                # remain in hangar
                print('load_ship -- ship is full and hangar has more items')
                return 1  # 1 indicating ship is full but more items remain
                # in hangar
