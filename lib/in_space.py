...
def select_waypoint():  # click on current waypoint in overview by looking for either station or stargate icons
    ...
    station_waypoint_icon = pyautogui.locateCenterOnScreen('station_waypoint_icon.png', confidence = 0.9)  # look for station icon
    while station_waypoint_icon is None:
        ...
        stargate_waypoint_icon = pyautogui.locateCenterOnScreen('stargate_waypoint_icon.png', confidence = 0.9)  # if station icon not found, look for stargate icon
        if stargate_waypoint_icon is None:
            ...
            pyautogui.moveTo((stargate_waypoint_iconx + (random.randint(-8,8))), (stargate_waypoint_icony + (random.randint(-8,8))), move_time(), mouse_path())  # clicks the center of where the button was found
            click()
            select_jump_button()
...
def detect_dock_or_jump():  # check if client has docked or jumped
    loopnum = 0  # set a loop number variable to determine how many loops of each imagesearch in total are run
    undock_icon = pyautogui.locateCenterOnScreen('undock_icon.png', confidence = 0.9)  # look for undock icon to indicate a dock has been made
